"""PDF 파서. Docling(표 보존) 1순위, 실패 시 Claude PDF 입력 폴백.
...
def _parse_with_claude(pdf_bytes: bytes) -> str:
    """Claude PDF 입력 폴백. Docling 실패 시 사용."""
    import anthropic
    import base64
...
def parse(
    ...
    try:
        ...
        print(f"[pdf:{source_yaml_id}] Docling 실패, Claude 폴백: {e}")
        try:
            raw_text = _parse_with_claude(pdf_bytes)
            print(f"[pdf:{source_yaml_id}] Claude 폴백 성공 ({len(raw_text)}자)")
        except Exception as e2:
            print(f"[pdf:{source_yaml_id}] Claude 폴백도 실패: {e2}")
